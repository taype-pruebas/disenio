...
export const createUserOrm = async (user) => {
    try {
        ...
        await userModel.findOne({user_email: user.user_email}).then(user => data = user).catch(err => {
            console.log(err);
            error = err})
            console.log(data);
        ...
        await userModel.create({...user, user_password: passwordEncript}).then(user => data = user).catch(err => {throw new Error(err || "No se pudo crear usuario")})
console.log(data);
        return {
